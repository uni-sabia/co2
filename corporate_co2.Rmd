...
The resulting dataset contains names of companies subjected to carbon pricing, their sectors, annual carbon emissions and allowances that they received for free from 2015 until 2019. Before we begin our analysis, let us identify the top 10 carbon-emitting companies in 2019.
...
co2_2019 <- co2 %>% filter(year==2019) %>% arrange(desc(emissions_t)) %>%
  select(-sector_kor, -energy_consumption_tj, -year, allowance_t)

head(co2_2019, 10) %>% kbl(caption="Top 10 carbon-intensive South Korean companies in 2019") %>%
  kable_classic()
```

POSCO,  Korea South-East Power, Korea Southern Power Co.Ltd., Korea Western Power, Korea Midland Power, Hyundai Steel, Samsung Electronics, Ssanyong Cement and S-Oil emitted the most carbon dioxide in South Korea. 5 of them are electricity generation companies. This is not surprising because [less than 5% of the electricity comes from renewable sources](https://www.index.go.kr/unify/idx-info.do?idxCd=4292). 

How were these companies affected by the K-ETS system since its inception in 2015? The following graph shows the change in annual emissions in the last 5 years. 

```{r}
posco_co2 <- co2 %>% filter(name_eng == "POSCO") 

ggplot(posco_co2, aes(x=year)) + 
  geom_line(aes(y=emissions_t)) +
  geom_line(aes(y=allowance_t), color="red") 
```
